...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // 🚨 FIX: Check if email is accidentally 'Double Wrapped'
    if (typeof email === 'object' && email !== null) {
        console.log("⚠️ Data was wrapped! Unwrapping now...");
        // Extract the real data from inside the object
        password = email.password;
        email = email.email; 
    }

    console.log("👉 FINAL EMAIL CHECK:", email); // Should be just text now

    // 2. Query with the clean text
    const [users] = await pool.query(
      'SELECT * FROM users WHERE email = ?', 
      [email] // Now this is definitely a string
    );
